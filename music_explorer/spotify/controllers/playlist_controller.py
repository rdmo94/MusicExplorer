...
class GetPlaylistGenresView(APIView):
    ...
    def get_artist_genres(self, spotify:Spotify, artists:list[str]) -> list[str]:
        genres = []
        artist_ids_chunks = self.split_list(artists, 50) #chunks of max 50 artist_ids
    ...
    def post(self, request):
        ...
        if current_user_response.ok:
            ...
            for (playlist_name, playlist_id) in request.data.items():
                ...
                genre_ocurrences = self.combine_occurence_dicts(genre_ocurrences, current_playlist_ocurrence_dict)
